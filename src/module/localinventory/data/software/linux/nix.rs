...
pub fn run_inventory() -> Vec<FinalPackageStruct> {
    let output_nix = get_nix();
    parse_nix_output(output_nix)
}
...
fn get_nix() -> String {
    ...
    match Command::new("nix-store")
        ...
        .output() {
            ...
            Err(_) => {
                println!("nix-store command error");
                "".to_string()
            }
        }
}

fn parse_nix_output(output: String) -> Vec<FinalPackageStruct> {
